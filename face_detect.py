...
class FaceRecognizer():
    def __init__(self, training_dir = None, model_path = ""):
        self._model_path = model_path
        self._training_dir = training_dir
        self._predictor = None
        self._temp_file = 'temp.jpg'
        self._image_data = None
        self._test_image_store_location = r""
    ...
    def predict_in_video(self, video_path, show_video = True, frame_skip_number = 50.0, save_test_frame = False, highlight_face = False ):
        ...
        while success:
            ...
            if show_video:
                res = cv2.resize(read,None,fx=0.2, fy=0.2, interpolation = cv2.INTER_CUBIC)
...
if __name__ == '__main__':
    ob = FaceRecognizer(training_dir = r'', model_path = 'new_recog.model')
    clf = ob.train(image_face_only = True, load_saved_images = True)
    ob.load_predictor(knn_clf = clf)
    ob.predict_in_video('', show_video = False, frame_skip_number = 50.0, save_test_frame = True, highlight_face = True)
    #preds = ob.predict(r"")
    #print(preds)
